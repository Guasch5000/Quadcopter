...
int angulo_x;
int angulo_y;
int diff_motor;               //diferencia entre los motores del mismo EIR
unsigned long time;
int dif_time;
uint16_t angulo_z;
int Const;
...
static void save_data() //Escribimo s en DataFlash
{
  ...
  //DataFlash.WriteInt(Const);  //Escribimos diferencia motores
...
{
  int vel_hold = 60; //Velocidad normal de los motores
  int vel_der;
  int vel_izq;
...
//Calculo salida P y D del controlador
  int vel_proportional = (num_Kp * angulo_y) / den_Kp;                //proporcional->angulo actual(angulo_y); derviativo-> velocidad angular(giroscopio)
  //MOD int vel_derivative = (num_Kd * (int)gyro.y) / den_Kd;               //gyro.y debuelve float, necesario pasar a int
